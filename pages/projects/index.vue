...
const selectedProjectModal = ref(null)
const isLoadingTareasModal = ref(false)
const isLoadingDocumentsModal = ref(false)
const projectTareasModal = ref([])
const projectDocumentsModal = ref([])
...
function openProjectModal(project) {
  selectedProjectModal.value = project
  showProjectModal.value = true
  loadProjectTareasModal(project.id)
  loadProjectDocumentsModal(project.id)
}

function closeProjectModal() {
  showProjectModal.value = false
  selectedProjectModal.value = null
  projectTareasModal.value = []
  projectDocumentsModal.value = []
}

async function loadProjectTareasModal(projectId) {
  isLoadingTareasModal.value = true
  const tareasRef = collection(db, 'tareas')
  const querySnapshot = await getDocs(tareasRef)
  projectTareasModal.value = querySnapshot.docs
    .map(doc => ({ id: doc.id, ...doc.data() }))
    .filter(t => t.proyectoId === projectId)
  isLoadingTareasModal.value = false
}

async function loadProjectDocumentsModal(projectId) {
  isLoadingDocumentsModal.value = true
  const docsRef = collection(db, 'documents')
  const querySnapshot = await getDocs(docsRef)
  projectDocumentsModal.value = querySnapshot.docs
    .map(doc => ({ id: doc.id, ...doc.data() }))
